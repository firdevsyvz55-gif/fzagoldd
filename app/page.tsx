...
export default function Home() {
  ...
  const css = `
    ...
    *, *::before, *::after { margin:0; padding:0; box-sizing:border-box }
    ...
    body { font-family:'Jost',sans-serif; font-weight:300; background:#faf9f7; color:#001a3d; overflow-x:hidden }
    ...
    .nav.solid .nav-logo-name { color:#001a3d }
    ...
    .nav.solid .burger span { background:#001a3d }
    ...
    /* ── TICKER ── */
    .ticker-wrap { background:#001a3d; padding:10px 0; overflow:hidden; border-bottom:1px solid rgba(201,168,76,.08) }
    ...
    /* ── SECTIONS ── */
    .sec { padding:96px 6% }
    .sec-dark { background:#001233; color:#fff }
    .sec-light { background:#faf9f7; color:#001a3d }
    .sec-grey { background:#f4f3f1; color:#001a3d }
    ...
    .f-btn:hover { color:#001a3d }
    .f-btn.on { color:#001a3d; border-bottom-color:#c9a84c }
    ...
    .pcard-bdg { position:absolute; top:12px; left:12px; background:#001a3d; color:#fff; font-size:6px; letter-spacing:2px; padding:4px 9px; font-weight:400; text-transform:uppercase }
    ...
    .pcard-name { font-family:'Cormorant Garamond',serif; font-size:18px; color:#001a3d; margin-bottom:4px; line-height:1.2; font-weight:400 }
    ...
    .about-item-t { font-size:8px; letter-spacing:2.5px; text-transform:uppercase; color:#001a3d; margin-bottom:4px; font-weight:500 }
    ...
    .stat-box { background:#001233; padding:28px 24px }
    ...
    /* ── FOOTER ── */
    .footer { background:#000d1f; padding:48px 6% 24px }
    ...
    .modal { background:#001233; border:1px solid rgba(255,255,255,.06); width:100%; max-width:620px; max-height:90vh; overflow-y:auto }
      ...
      <div className={`mobile-menu ${menuOpen ? 'open' : ''} ${navSolid ? 'mobile-menu-solid' : 'mobile-menu-dark'}`}>
        {[
          ...
          <a key={href as string} href={href as string}
            ...
            style={{ color: navSolid ? '#001a3d' : 'rgba(255,255,255,.85)', borderBottom: '1px solid rgba(201,168,76,.08)' }}>
            {label}
          </a>
        ))}
      </div>
        ...
        <div className="calc-wrap">
          <div className="calc-left">
            ...
            <select className="c-sel" value={calcKarat} onChange={e => setCalcKarat(parseFloat(e.target.value))}>
              <option value={0.9166} style={{ background: '#001a3d' }}>22 {T('Ayar', 'Karat')} — 916‰</option>
              <option value={0.75} style={{ background: '#001a3d' }}>18 {T('Ayar', 'Karat')} — 750‰</option>
              <option value={0.585} style={{ background: '#001a3d' }}>14 {T('Ayar', 'Karat')} — 585‰</option>
              <option value={0.375} style={{ background: '#001a3d' }}>9 {T('Ayar', 'Karat')} — 375‰</option>
            </select>
            ...
            <select className="c-sel" value={calcKdv} onChange={e => setCalcKdv(parseFloat(e.target.value))}>
              <option value={0} style={{ background: '#001a3d' }}>{T('KDV Dahil Değil', 'Excl. VAT')}</option>
              <option value={0.18} style={{ background: '#001a3d' }}>%18 KDV</option>
              <option value={0.20} style={{ background: '#001a3d' }}>%20 KDV</option>
            </select>
          </div>
